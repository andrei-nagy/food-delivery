...
export const Waiter = ({ url }) => {
  ...
  const statusHandler = async (event, orderId) => {
    const response = await axios.post(url + "/api/waiterorders", {
      orderId,
  ...
  return (
    <div className='list add flex-col order-item'>
      ...
      <div className="list-table">
        ...
        {list.filter(item => item.status === 'Pending').map((item, index) => (
          <div key={item._id} className={`order-item ${newRequests.includes(item) ? 'new-request' : ''}`}>
