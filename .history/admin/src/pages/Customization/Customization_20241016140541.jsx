...
// Funcție pentru generarea unui ID random
const generateRandomId = () => {
    return Math.random().toString(36).substring(2, 10); // Generează un ID aleatoriu de 8 caractere
};

const AdminCustomizationPage = ({ url }) => {
    const [customizationData, setCustomizationData] = useState({
        restaurantId: generateRandomId(), // Generăm un ID random pentru restaurant
        restaurantName: '',
        logoUrl: '',
    ...
    // Funcție pentru a verifica dacă restaurantId este valid
    const isValidObjectId = (id) => {
        return id && id.length === 8; // Verificăm dacă ID-ul are lungimea corectă
    };
    ...
    const fetchCustomizationData = async () => {
        setLoading(true);
        try {
            // Verificăm dacă restaurantId este valid înainte de a face cererea
            if (!isValidObjectId(customizationData.restaurantId)) {
