...
const Customization = ({ url }) => {
    ...
    const onImageUpload = (event) => {
        ...
        const imageUrls = files.map(file => URL.createObjectURL(file)); // Poți salva URL-urile temporare în starea imagini
        setImages(imageUrls);
        setData(prevData => ({ ...prevData, backgroundImages: files })); // Trimite fișierele la server pentru a fi încărcate
    };
 ...
 data.backgroundImages.forEach(image => {
    formData.append('backgroundImages[]', image); // Folosirea 'backgroundImages[]' pentru array
});
    ...
    return (
        <div className='add'>
            <form className='flex-col' onSubmit={onSubmitHandler}>
                ...
                <div className="add-product-headers flex-col">
                <p>Headers backgrounds</p>
                <input type="file" name="backgroundImages" multiple onChange={onImageUpload} />
</div>
