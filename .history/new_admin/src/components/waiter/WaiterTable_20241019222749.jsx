...
const WaiterTable = () => {
    ...
    const [newRequests, setNewRequests] = useState([]); // Cereri noi
    ...
    const fetchList = async () => {
        ...
        if (response.data.success) {
            ...
            // Verificăm dacă există cereri noi
            const pendingRequests = newList.filter(item => item.status === 'Pending');
            const newRequestsOnPage = pendingRequests.filter(item => !list.some(existingItem => existingItem._id === item._id));

            // Setăm cererile noi doar dacă sunt diferite de cele anterioare
            if (newRequestsOnPage.length > 0) {
                setNewRequests(prevRequests => [
                    ...prevRequests,
                    ...newRequestsOnPage.filter(item => !prevRequests.some(existingItem => existingItem._id === item._id))
                ]);
