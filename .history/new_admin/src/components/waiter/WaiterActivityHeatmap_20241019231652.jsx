    ...
    const fetchList = async (start, end) => {
        try {
            ...
            if (response.data.success) {
				...
				const updatedUserActivityData = Array.from({ length: 7 }, (_, i) => ({
             ...
             // Procesul de incrementare a orelor
newList.forEach(item => {
    ...
    if (hour >= 0 && hour < 4) {
        updatedUserActivityData[dayIndex]["0-4"]++;
    } else if (hour >= 4 && hour < 8) {
        updatedUserActivityData[dayIndex]["4-8"]++;
    } else if (hour >= 8 && hour < 12) {
        updatedUserActivityData[dayIndex]["8-12"]++;
    } else if (hour >= 12 && hour < 16) {
        updatedUserActivityData[dayIndex]["12-16"]++;
    } else if (hour >= 16 && hour < 20) {
        updatedUserActivityData[dayIndex]["16-20"]++;
    } else if (hour >= 20 && hour < 24) {
        updatedUserActivityData[dayIndex]["20-24"]++;
    }
});

// Aici setezi starea corect
setUserActivityData(updatedUserActivityData);
            } else {
