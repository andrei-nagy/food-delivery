...
const OrdersToCloseTable = () => {
    ...
    const handleAllPaymentStatusChange = async (paymentStatus) => {
    ...
    const handlePaymentStatusChange = async (orderId, paymentStatus) => {
        try {
            ...
            if (response.data.success) {
                ...
                // Optionally refresh data if necessary
                // fetchData(); 
            } else {
    ...
    const handleCheckOrderClick = (userOrder) => {
        setSelectedUserId(userOrder.tableNo); // Set selected user ID
                            ...
                            <tbody className='divide divide-gray-700'>
                                {relatedOrders.map((order) => (
                                    <tr key={order._id}>
                                        ...
                                        <td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100'>
    <select
        onChange={(e) => handlePaymentStatusChange(order._id, e.target.value === "true")} // Pass the order ID and new status
        value={order.payment ? "true" : "false"} // Current payment status value
        className={`text-white rounded-lg p-2 ${getStatusClass(order.payment)}`} // Style based on status
    >
        ...
        <option value="false">Not paid</option>
                    ...
                    onClick={() => handleAllPaymentStatusChange(true)} // Call the new function to change all payment statuses
                >
                    Payment completed
                </button>
