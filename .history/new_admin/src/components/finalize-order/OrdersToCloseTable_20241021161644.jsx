...
const OrdersToCloseTable = () => {
    ...
    const [showTooltip, setShowTooltip] = useState(false); // State pentru tooltip

    const handleSelectClick = (currentOrder) => {
        if (!currentOrder.payment) {
            setShowTooltip(true); // Afișează tooltip-ul
            setTimeout(() => {
                setShowTooltip(false); // Ascunde tooltip-ul după 2 secunde
            }, 2000);
        }
    };
                        ...
                        {currentOrders.map((order) => (
                            ...
                            >
                                ...
                                <td className='px-6 py-4 whitespace-nowrap text-sm font-medium'>
                                    <select
                                        onClick={handleSelectClick(order)} // Apelează funcția la click
                                        onChange={(e) => {
                                            // Check if the order is paid
                                            if (!order.payment) {
                                                return; // Previne schimbarea stării dacă order.payment este false
                                            }
                                            // If paid, proceed to change the status
                                            handleIsActiveChange(order.user?._id, e.target.value === "true"); // Trimite true/false în funcție de selecție
                                        }}
                                        value={order.user?.isActive === true ? "true" : "false"} // Dacă isActive este true, selectează "Active"
                                        className={`text-white rounded-lg p-2 ${getStatusClass(order.user?.isActive)}`} // Stilizează în funcție de isActive
                                        disabled={!order.payment} // Dezactivează dacă payment este false
                                    >
                                        <option value="true">Active</option>
                                        <option value="false">Inactive</option>
                                    </select>
                                    {/* Afișează tooltip-ul dacă order.payment este false */}
                                    {showTooltip && (
                                        <p className="text-red-500 text-sm mt-1 absolute bg-white border border-red-500 rounded p-1">
