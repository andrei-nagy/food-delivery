...
const OrdersToCloseTable = () => {
    ...
    <select
        onChange={(e) => {
            if (!order.payment) {
                setShowNotification(true); // Afișează notificarea
                return; // Previne schimbarea stării
            }
    ...
    {/* Afișează notificarea dacă order.payment este false */}
    {showNotification && !order.payment && (
        <div className="mt-2 text-red-600 bg-red-100 p-2 rounded">
            Order is not paid yet. Please complete the payment before changing the status.
        </div>
    )}
</td>
