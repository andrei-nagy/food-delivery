...
const OrdersToCloseTable = () => {
    ...
    const handleCheckOrderClick = (order) => {
        ...
        console.log('combined orders sus')
        const related = combinedOrders.filter(o => o.userId === order.userId); // Filtrează comenzile pe baza tableNo
    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50'>
        <div className='bg-gray-800 rounded-lg p-6 w-3/4 md:w-2/3 lg:w-1/2 max-h-[80vh] overflow-y-auto'>
            <h2 className='text-lg font-semibold text-gray-100 mb-4'>Related Orders for Table {relatedOrders[0]?.tableNo}</h2>
            <table className='min-w-full divide-y divide-gray-700'>
                <thead>
                    <tr>
                        <th className='px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider'>
