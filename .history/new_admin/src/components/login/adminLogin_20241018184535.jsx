  ...
  return (
    <section className="h-screen flex flex-col md:flex-row justify-center space-y-10 md:space-y-0 md:space-x-16 items-center my-2 mx-5 md:mx-0 md:my-0">
      ...
      <div className="md:w-1/3 max-w-sm">
        ...
        <form onSubmit={handleSubmit}> {/* Adaugă un formular */}
          <input
          ...
          />
          {isRegister && ( // Câmpul pentru Security Token este vizibil doar în mod de înregistrare
            <input
              className="text-sm w-full px-4 py-2 border border-solid border-gray-300 rounded mt-4"
              type="text"
              placeholder="Security Token"
              value={securityToken}
              onChange={(e) => setSecurityToken(e.target.value)} // Actualizează Security Token
            />
          )}
          ...
          <div className="text-center md:text-left">
            <button
              ...
              type="submit"
            >
              {isRegister ? "Register" : "Login"} {/* Schimbă textul în funcție de starea curentă */}
            </button>
          </div>
        </form>
        <div className="mt-4 font-semibold text-sm text-slate-500 text-center md:text-left">
          {isRegister ? ( // Schimbă textul în funcție de starea curentă
            <>
