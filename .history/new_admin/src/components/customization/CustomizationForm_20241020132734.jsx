...
const CustomizationPage = () => {
   ...
   const onSubmitHandler = async (event) => {
    ...
    // Only append a new image if it's selected
    if (image) {
        formData.append('image', image);
    } else if (isExistingCustomization && data.image) {
        // If there's an existing customization and no new image uploaded,
        // Do not append the image at all
        // Optionally, you could send a signal here if you need to indicate no change to the image
        // formData.append('image', data.image); // This line should be removed
    }
    ...
    try {
        ...
        if (response.data.success) {
            setData({
                image: '', // Reset other fields after submission
                restaurantName: '',
                primaryColor: '#ffffff',
                secondaryColor: '#000000',
                slogan: '',
            ...
            toast.success(isExistingCustomization ? 'Customization updated successfully!' : 'Customization added successfully!');
