...
const OrdersTable = () => {
    ...
    const handleSearch = (e) => {
        ...
        const filtered = orders.filter(
            (order) =>
                order.userData?.orderNumber.toString().toLowerCase().includes(term) || // Căutare după orderNumber
                order.userData?.customer?.toLowerCase().includes(term) // Căutare după customer
        );
                ...
                <table className='min-w-full divide-y divide-gray-700'>
                    ...
                    <tbody className='divide divide-gray-700'>
                        {filteredOrders.map((order) => (
                            <motion.tr
                                key={order.userData?.orderNumber} // Folosește orderNumber ca și key
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                transition={{ duration: 0.3 }}
                            >
                                <td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100'>
                                    {order.userData?.orderNumber || "Unknown"} {/* Afișează orderNumber */}
                                </td>
                                <td className='px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-100'>
