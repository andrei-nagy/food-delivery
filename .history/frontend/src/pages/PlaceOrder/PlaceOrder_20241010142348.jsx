...
const PlaceOrder = () => {
  ...
  const placeOrder = async (event) => {
    ...
    if (paymentMethod === 'creditCard') {
      console.log('plata credit card online')
      ...
      let response = await axios.post("https://localhost:4000/api/order/place-cash", orderData, { headers: { token } });
      console.log({
        url: url,
        orderData: orderData,
        response: response
      ...
      if (response.data.success) {
        ...
        // navigate("/thank-you"); // Redirecționează utilizatorul la o pagină de mulțumire
        navigate('/')
      } else {
