...
const App = () => {
  ...
  const [isValidUser, setIsValidUser] = useState(null); // Starea pentru validitatea userului
  ...
  useEffect(() => {
    if (location.pathname === '/register') {
      ...
      setTimeout(() => {
        setLoading(false); // Dezactivează loading după 40 secunde sau când datele sunt gata
  ...
  return (
    <>
      ...
      {!loading && (
        <>
          <CheckUser url="http://localhost:4000" onValidation={handleValidation} /> {/* Verificare utilizator */}
          {/* Afișăm aplicația doar dacă utilizatorul este valid */}
          {isValidUser === true && (
            <>
          ...
          {/* Dacă utilizatorul nu este valid, afișăm pagina de Welcome */}
          {isValidUser === false && (
            <Routes>
              <Route path="/welcome" element={<Welcome />} />
            </Routes>
