...
const ModalCart = ({ show, onClose, cartItems, food_list, checkoutHandler }) => {
  ...
  const updateQuantity = (itemId, newQuantity) => {
    if (newQuantity <= 0) {
      // Dacă cantitatea ajunge la zero, poți să îndepărtezi produsul din coș
      removeFromCart(itemId);
    } else {
      // Dacă cantitatea este mai mare decât zero, actualizezi cantitatea
      cartItems[itemId] = newQuantity;
      // Ar trebui să apelezi funcția care actualizează starea în StoreContext
      // De exemplu, dacă ai un context care ține starea coșului:
