...
const ModalMyOrders = ({ show, onClose }) => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                setOrders(response.data.data); // Folosim orders direct din data
            } catch (error) {
    ...
    const renderOrderItems = () => {
        return orders.map((order) => {
            // Calculăm numărul de produse cu statusurile corespunzătoare
            const statusCount = { Delivered: 0, "Food processing": 0 };
            order.items.forEach((item) => {
                statusCount[item.status] = (statusCount[item.status] || 0) + item.quantity;
            });

            const totalQuantity = order.items.reduce((sum, item) => sum + item.quantity, 0); // Totalul cantităților
                ...
                <div key={order._id} className='order-item'>
                    <div className="order-header">
                        ...
                        <p className="order-status">
                            {getStatusText(statusCount, totalQuantity)}
                        </p>
                    </div>
