...
const FoodItemCategory = ({ id, name, price, description, image, isBestSeller, isNewAdded, isVegan, category }) => {
    ...
    const [isNutritionModalOpen, setIsNutritionModalOpen] = useState(false);  // <-- nou
  ...
  useEffect(() => {
    if (isModalOpen) {
      ...
      // Restabilește scroll când modalul e închis
      document.body.style.overflow = '';
    }

    // Cleanup, când componenta se demontează, să fie sigur că scroll-ul revine la normal
    return () => {
      document.body.style.overflow = '';
    };
  }, [isModalOpen]);
