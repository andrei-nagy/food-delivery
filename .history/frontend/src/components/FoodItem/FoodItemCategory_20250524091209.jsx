...
const FoodItemCategory = ({ id, name, price, description, image, isBestSeller, isNewAdded, isVegan, category }) => {
    ...
    // Păstrăm cantitatea inițială când se deschide modalul
    ...
    const handleAddToOrder = () => {
        ...
        if (diff > 0) {
            ...
            // Eliminăm diferența în minus
            removeFromCart(selectedFood.id, -diff);
        } else {
            // Dacă nu s-a modificat cantitatea, poate vrei să actualizezi specialInstructions
            // Dacă funcția addToCart actualizează și instrucțiunile, poți apela și aici:
            // addToCart(selectedFood.id, 0, specialInstructions);
        }
        ...
        <>
            <div className='food-item-category' onClick={openFoodModal} style={{ cursor: 'pointer' }}>
                <div className="food-item-img-container">
                    ...
                    {isBestSeller && (
                        <img className='best-seller-badge' src={assets.bestseller_icon} alt="Best Seller" />
                    )}
