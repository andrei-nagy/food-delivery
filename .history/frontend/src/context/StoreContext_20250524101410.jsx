...
const StoreContextProvider = (props) => {
    ...
    const updateCartItemQuantity = async (itemId, newQuantity, specialInstructions = "") => {
        console.log("updateCartItemQuantity called with:", { itemId, newQuantity, specialInstructions });
        ...
        if (newQuantity <= 0) {
            ...
            console.log(`Setting quantity for item ${itemId} to ${newQuantity}`);
            updatedCart[itemId] = newQuantity;
        }
    
        // Actualizăm local UI-ul rapid
        setCartItems(updatedCart);
        console.log("Local cartItems after local update:", updatedCart);
        ...
        if (token) {
            try {
                ...
                if (response.data.cartData) {
                    console.log("Updating local cartItems with server response:", response.data.cartData);
                    setCartItems(response.data.cartData);
    ...
    const loadCartData = async (token) => {
        try {
          ...
          console.log("loadCartData - server response:", response.data);
          if (response.data.cartData) {
            setCartItems(response.data.cartData);
            console.log("loadCartData - setCartItems:", response.data.cartData);
