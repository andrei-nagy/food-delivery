...
const getOrderRating = async (req, res) => {
    ...
    try {
      ...
      if (order) {
        console.log('am gasit order')
        res.json({ success: true, rating: order.orderRating || 0 });
      } else {
        console.log('am gasit order 1')
        res.json({ success: false, message: "Order not found" });
      }
    } catch (error) {
        console.log('am gasit order 2')
...
export { placeOrderCash, placeOrder, verifyOrder, userOrders, listOrders, updateStatus,updateOrderRating, getOrderRating }
