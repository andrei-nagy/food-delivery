...
const updateCustomization = async (req, res) => {
    ...
    try {
        const customization = await customizationModel.findOneAndUpdate(
            ...
            { new: true, upsert: true } // Creează o personalizare nouă dacă nu există (upsert)
        );
...
const getCustomization = async (req, res) => {
    try {
        // Obține datele de personalizare din baza de date pentru un anumit restaurant
        ...
        // Verifică dacă există personalizarea
        if (!customizationData) {
            return res.status(404).json({
                success: false,
                message: 'Customization data not found for this restaurant.',
            });
        }
