...
// Funcție pentru a șterge utilizatorii cu token-uri expirate
const deleteExpiredUsers = async () => {
    try {
        // Obține ora curentă
        const now = new Date();

        // Șterge utilizatorii cu tokenExpiry mai mic decât ora curentă
        const result = await userModel.deleteMany({ tokenExpiry: { $lt: now } });
   
        console.log(`${result.deletedCount} utilizatori cu token expirate au fost șterși.`);
    } catch (error) {
        console.error("Eroare la ștergerea utilizatorilor expirate:", error);
    }
};
...
import tableModel from "../models/tableModel.js";  // Importăm modelul pentru mese
...
const autoLogin = async (req, res) => {
    ...
    try {
        ...
        if (!table) {
            ...
            // Dacă există, actualizează statusul la "occupied"
            table.status = 'occupied';
            table.userId = user._id;
            table.createdOn = new Date();  // Actualizează ora de ocupare
        }
...
const autoRegister = async (req, res) => {
    ...
    console.log("Numele utilizat:", name);  // Verificare nume folosit
    try {
        // Caută un utilizator existent pe baza tableNumber
        let user = await userModel.findOne({ tableNumber });
        ...
        const newUser = new userModel({
            ...
            tableNumber: tableNumber,
            activeSessions: 1  // Prima sesiune activă
        });
console.log(newUser)
...
export { loginUser, registerUser, autoLogin, autoRegister, deleteExpiredUsers };