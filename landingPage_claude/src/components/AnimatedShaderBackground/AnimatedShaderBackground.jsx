...
export default function AnimatedShaderBackground({ className = '', style = {}, children }) {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        float noise(vec2 p) {
          ...
          u = u * u * (3.0 - 2.0 * u);
          float a = rand(ip), b = rand(ip+vec2(1,0)), c = rand(ip+vec2(0,1)), d = rand(ip+vec2(1,1));
  ...
  return (
    <div ref={containerRef} className={`shader-bg ${className}`.trim()} style={style}>
      {children && <div className="shader-bg__content">{children}</div>}
    </div>
  );
}