...
// Helper function to get the first day (Monday) and last day (Sunday) of the current week
const getWeekRange = () => {
    const today = new Date();
    const firstDayOfWeek = new Date(today);
    const lastDayOfWeek = new Date(today);

    // Set to the previous Monday (or today if today is Monday)
    firstDayOfWeek.setDate(today.getDate() - today.getDay() + (today.getDay() === 0 ? -6 : 1)); // 1 for Monday, 0 is Sunday
    // Set to the next Sunday
    lastDayOfWeek.setDate(firstDayOfWeek.getDate() + 6); // 6 for Sunday
...
const DailySalesTrend = () => {
    ...
    const url = 'https://api.orderly.ro'; // Your API endpoint
    ...
    const fetchSalesData = async () => {
        try {
            ...
            // Filter to include only orders with status 'Delivered' and within the current week
            const deliveredOrders = orders.filter(order => {
                const orderDate = new Date(order.date || order.createdOn); // Ensure correct date is used
                return order.status === 'Delivered' && orderDate >= firstDayOfWeek && orderDate <= lastDayOfWeek;
            });
            ...
            deliveredOrders.forEach((order) => {
                const orderDate = new Date(order.date || order.createdOn);
                const day = getDayOfWeek(orderDate); // Extract day from date
