...
import { 
    X, Upload, Clock, Mail, Phone, Shield, Trash2, Globe, CreditCard, 
...
const CustomizationPage = () => {
    ...
    const fetchCustomization = async () => {
        try {
            console.log('Fetching customization from:', `${url}/admin/personalization/get`);
            ...
            if (response.data.success && response.data.data) {
                ...
                if (customizationData.openingHours) {
                    console.log('Opening hours found:', customizationData.openingHours);
                    setSchedule({
                        weekdays: {
    ...
    const onSubmitHandler = async (event) => {
        ...
        if (data._id) {
            ...
            console.log('Added ID to formData:', data._id);
        }
        ...
        if (image && image instanceof File) {
            ...
            console.log('Added existing image to formData:', data.image);
        } else if (image && typeof image === 'string') {
            // If image is a string from backend
            formData.append('image', image);
        }

        // Add all other fields - CORECTAT: Folosește data, nu editData
        const fieldsToSend = {
        ...
        Object.entries(fieldsToSend).forEach(([key, value]) => {
            ...
            console.log(`Added field ${key}:`, finalValue);
        });

        // Add opening hours
        formData.append('openingHours', JSON.stringify(schedule));
        console.log('Added openingHours:', schedule);
        ...
        try {
            ...
            console.log(`Sending ${method} request to:`, `${url}/admin/personalization/${endpoint}`);
    ...
    // Iconiță custom pentru currency (folosește simbolul curent)
        ...
        >
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl overflow-hidden">
                ...
                <form className="p-6" onSubmit={onSubmitHandler}>
                        ...
                        <div className="mb-4 p-3 bg-gray-900/50 rounded-lg border border-gray-700">
                            <div className="flex items-center justify-between">
                                <div className="text-sm">
                                    ...
                                    <span className={isExistingCustomization ? 'text-green-400' : 'text-yellow-400'}>
                                        {isExistingCustomization ? 'UPDATE' : 'CREATE'}
                                    </span>
                                    <span className="ml-4 text-gray-400">ID: </span>
                                    <span className="text-blue-400">
                                        {data._id ? data._id.substring(0, 8) + '...' : 'No ID'}
                                    </span>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => {
                                        console.log('Current state:', { data, schedule, image, isExistingCustomization });
                                        toast.info('State logged to console', { theme: "dark" });
                                    }}
                    ...
                    {activeTab === 'financial' && (
                        <div className="space-y-8">
                            <h3 className="text-xl font-semibold text-white mb-6">Financial Settings</h3>
                    ...
                    {activeTab === 'features' && (
                        <div className="space-y-8">
                            <h3 className="text-xl font-semibold text-white mb-6">Features & Services</h3>
                            
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <label className="flex items-center space-x-3 p-4 bg-gray-700/30 rounded-xl cursor-pointer hover:bg-gray-700/50 transition-all duration-200">
                                    <input
                                        type="checkbox"
                                        name="reservationEnabled"
                                        checked={data.reservationEnabled}
                                        onChange={onChangeHandler}
                                        className="w-5 h-5 text-blue-500 rounded focus:ring-blue-600 focus:ring-offset-gray-900"
                                    />
                                    <div>
                                        <span className="font-medium text-white">Reservations</span>
                                        <p className="text-sm text-gray-400">Allow table reservations</p>
                                    </div>
                                </label>
                                ...
                                <label className="flex items-center space-x-3 p-4 bg-gray-700/30 rounded-xl cursor-pointer hover:bg-gray-700/50 transition-all duration-200">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-400">Allow takeaway orders</p>
                                    </div>
                                </label>
                                ...
                                <label className="flex items-center space-x-3 p-4 bg-gray-700/30 rounded-xl cursor-pointer hover:bg-gray-700/50 transition-all duration-200">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-400">Enable delivery service</p>
                                    </div>
                                </label>
                                ...
                                <label className="flex items-center space-x-3 p-4 bg-gray-700/30 rounded-xl cursor-pointer hover:bg-gray-700/50 transition-all duration-200">
                                    ...
                                    />
                                    <div>
                                        <span className="font-medium text-white">Customer Ratings</span>
                                        <p className="text-sm text-gray-400">Allow dish ratings</p>
                                    </div>
                                </label>
                                ...
                                <label className="flex items-center space-x-3 p-4 bg-gray-700/30 rounded-xl cursor-pointer hover:bg-gray-700/50 transition-all duration-200">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-400">Enable tipping feature</p>
                                    </div>
                                </label>
                                ...
                                <label className="flex items-center space-x-3 p-4 bg-gray-700/30 rounded-xl cursor-pointer hover:bg-gray-700/50 transition-all duration-200">
                                    ...
                                    />
                                    <div>
                                        <span className="font-medium text-white">QR Code Menu</span>
                                        <p className="text-sm text-gray-400">Enable QR code scanning</p>
                                    </div>
                                </label>
                            </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            required
                                        />
                                        <span className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400">hours</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {activeTab === 'appearance' && (
                        <div className="space-y-8">
                            <h3 className="text-xl font-semibold text-white mb-6">Appearance & Design</h3>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <input
                                            ...
                                            value={data.primaryColor}
                                            onChange={onChangeHandler}
                                            className="flex-1 px-4 py-3 bg-gray-700/50 border border-gray-600 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                                        />
                                    </div>
                                </div>
                    ...
                    <div className="flex justify-between items-center pt-8 mt-8 border-t border-gray-700">
                        ...
                        <div className="flex space-x-4">
                            <button
                                ...
                                className="px-6 py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-xl transition-all duration-200 font-medium"
