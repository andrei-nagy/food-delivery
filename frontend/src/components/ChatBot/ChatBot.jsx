...
const ChatBot = ({ show, onClose }) => {
  ...
  // NEW FUNCTION: Show payment options
  ...
  const handleQuickReply = (reply) => {
    if (reply.action === "view_full_menu") {
      ...
      // Handle payment quick reply by showing payment options
      showPaymentOptions();
    } else {
  ...
  const handleQuickAction = (actionType) => {
    ...
    switch (actionType) {
      case "recommendation":
        ...
        break;
      case "payment":
        // Instead of directly sending a message, show payment options
        showPaymentOptions();
        return; // Return early to avoid sending message
      case "hours":
  ...
  const processConfirmationResponse = (response) => {
    ...
    if (!pendingLanguageChange && !pendingWaiterRequest) {
      if (/(card|pos|terminal|contactless)/i.test(lowerResponse)) {
        ...
        setTimeout(() => {
          confirmWaiterRequest("payment", 
            language === "ro" ? "Plată cu cardul" : "Card payment"
          );
        }, 500);
        return true;
        ...
        setTimeout(() => {
          confirmWaiterRequest("payment", 
            language === "ro" ? "Plată cash" : "Cash payment"
          );
        }, 500);
        return true;
      }
  ...
  const findBestMatch = async (question, lang) => {
    ...
    if (lang === "ro") {
      ...
      if (isPaymentQuestion) {
        // Show payment options for payment questions
        setTimeout(() => {
          showPaymentOptions();
        }, 100);
        return "";
